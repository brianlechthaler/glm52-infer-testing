...
- 262K context (`MAX_MODEL_LEN`) and a configurable output cap
...
| `limit.context` | Max context tokens; keep `<=` `MAX_MODEL_LEN` (default 262144) |
...
| Context | `MAX_MODEL_LEN` (default 262144) bounds total context. opencode sends full session history, so long sessions approach this; raise `MAX_MODEL_LEN` with VRAM headroom, or start new sessions. |
